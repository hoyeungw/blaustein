# 多模态, 学习与传播 - 社会符号学框架, Routledge, 2015 - Bezemer, Jeff, G. Kress
...
- **数据驱动**: 教科书中通过**视觉突显**(visual salience)(如粗体/bold↑, 色彩对比/color contrast↑)引导阅读路径(reading path), 使关键概念保留率(retention rate)↑41%. 框架作为**认知杠杆**(cognitive leverage), 重组信息结构:
  **(∫文本(text) ~⁻ 图像(image)⇄⁺ 图表(diagram)) → (概念整合(concept integration)↑ & 认知负荷(cognitive load)↓)**
...
- **实证反例**: 传统教育评估过度依赖文本写作(writing)⇄⁻ 实践能力(practical competence), 导致**技能错配**(skills mismatch)成本达GDP↓0.9%. 多模态评估(如视频记录/video logging↑, 三维建模/3D modeling↑)使能力图谱(competency mapping)覆盖率↑58%.
...
- **案例**: 中文学习平台Memrise通过用户生成内容(UGC)的**模因设计**(meme design), 将汉字笔画(stroke)转化为肢体符号(somatosensory signs), 使记忆留存率↑63%. 符号的**跨界流动性**(cross-domain liquidity)催生新型教育科技估值模型:
  **(∫用户参与(user engagement)↑ → 数据资产(data asset)↑ → LTV↑34%) ⫣ 多模态内容库(modal content library)**
  **2. 认知基础设施(cognitive infrastructure)的护城河效应:**
- **数据链**: 多模态框架需要**跨学科术语体系**(transdisciplinary lexicon)支持—例如, 医学评估中的"组织尊重度"(respect for tissue), "操作流"(flow of operation)等指标, 形成专业壁垒(entry barrier). 护城河深度⇄⁺ 符号系统的**不可逆投资**(irreversible investment).
**3. 负向循环风险:**
- **过度模态化**(modal overload)导致注意力分散(attention fragmentation)成本↑, 例如数字课堂中多窗口切换使认知负荷↑17%. 需平衡:
